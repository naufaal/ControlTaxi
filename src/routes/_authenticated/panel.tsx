...
export const Route = createFileRoute("/_authenticated/panel")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Ingresos y gastos del día, llegadas de Barajas y trenes de alta velocidad." },
    ],
  }),
  component: Panel,
});
...
function Panel() {
  ...
  // Pre-procesamiento de transportes
  ...
  return (
    <main className="min-h-dvh bg-background pb-28">
      ...
      {search.modal === "documentos" && <VentanaDocumentosModal onCerrar={cerrarModal} currentUserId={currentUserId} />}
...
// ============================================================================
// COMPONENTES REUTILIZABLES (UI & MODALS)
// ============================================================================
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <label className="flex-1 flex items-center justify-center gap-2 h-12 rounded-xl border border-input bg-secondary px-4 text-xs font-medium text-foreground cursor-pointer hover:bg-secondary/80 transition-colors truncate">
              ...
              <input type="file" accept=".pdf,image/*" onChange={(e) => setArchivo(e.target.files?.[0] || null)} className="hidden" />
            </label>
            <button type="submit" disabled={subiendo || !archivo} className="h-12 px-5 rounded-xl bg-primary text-primary-foreground font-semibold text-xs shadow disabled:opacity-50 shrink-0">
